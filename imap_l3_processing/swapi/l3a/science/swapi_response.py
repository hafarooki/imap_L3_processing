...
class SWAPIResponse:
    ...
    _gf_cache: dict = field(default_factory=dict, init=False, repr=False, compare=False)
    ...
    def geometric_factor(
        self, esa_voltage: float, charge_per_proton_charge: float = 1.0
    ) -> float:
        """G(V, q) in cm² · sr · eV. Delegates to `calculate_geometric_factor`."""
        from imap_l3_processing.swapi.l3a.science.geometric_factor import (
            calculate_geometric_factor,
        )

        return calculate_geometric_factor(self, esa_voltage, charge_per_proton_charge)
    ...
    def from_files(
        ...
        response._oa_active_el_range = _active_el_range(
            response._min_OA_boundary, _TARGET_ELEVATIONS
        )
        response._sg_active_el_range = _active_el_range(
            response._min_SG_boundary, _TARGET_ELEVATIONS
        )
        return response
