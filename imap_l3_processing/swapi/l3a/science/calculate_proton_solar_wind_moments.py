...
_BASIN_REFINE_REL_TOL = 1e-3
_VERIFIER_RMSE_RATIO_THRESHOLD = 10
...
# xtol fires before ftol on this problem, so loosening ftol is redundant.
_LM_XTOL = 1e-3
...
_STATE_LOG_DENSITY_IDX = 0
_STATE_LOG_TEMPERATURE_IDX = 1
_STATE_VELOCITY_SLICE = slice(2, 5)
...
def _get_initial_guess(
    ...
    model_count_rate_at_unit_density = _model_count_rates(
        1.0,
        temperature,
        bulk_velocity_rtn,
        passband_grids,
    ...
    density = _optimal_density_scale(model_count_rate_at_unit_density, count_rate)
...
def calculate_integral(
    ...
    sg_rate = _integrate_sunglasses_region(
        grid,
        sw_params,
        central_speed,
    ...
    vv_rate = _integrate_vanes_vignetting_regions(
        grid,
        sw_params,
        central_speed,
        central_effective_area,
    ...
    oa_rate = _integrate_open_aperture_regions(
        grid,
        sw_params,
        central_speed,
        central_effective_area,
...
def _integrate_sunglasses_region(
    ...
    min_el, max_el, min_az, max_az = _integration_window(
        sw_params, _REGION_SUNGLASSES, grid, central_speed
    )
...
def _maxwellian_exponential(
    sw_params: SWParams, cos_view_angle_to_bulk: float, speed: float
) -> float:
    return math.exp(
        -(
            speed**2
...
def _trim_oa_azimuth_by_integrand(
    ...
    transmission_times_maxwellian = np.exp(
        -relative_velocity_sq / (2.0 * sw_params.thermal_speed**2)
    )
...
def _optimize(
    ...
    spin_axis_rtn = _average_spin_axis_in_rtn(rotation_matrices)
    final_result = _refine_basin_via_k_rotation(
        first_lm_result,
        run_lm,
        count_rate,
        passband_grids,
    ...
    density_sigma, temperature_sigma, velocity_covariance = (
        _uncertainties_from_residual_scaled_jacobian(final_result, density, temperature)
    )
...
def _refine_basin_via_k_rotation(
    first_lm_result,
    run_lm,
    count_rate: ndarray,
    passband_grids: numba.typed.List,
    ...
    current_mse = float(np.mean(first_lm_result.fun**2))

    for _ in range(_MAX_BASIN_REFINE_ITERS):
        rotated_velocity, rotated_density, verifier_mse = _best_k_rotation_seed(
            current_result,
            count_rate,
            passband_grids,
            central_speeds,
        ...
        verifier_rules_out_competing_basin = (
            verifier_mse >= current_mse * _VERIFIER_RMSE_RATIO_THRESHOLD ** 2
        )
        if verifier_rules_out_competing_basin:
            break

        restart_result = _lm_restart_from_rotated_seed(
            run_lm, current_result, rotated_velocity, rotated_density
        )
        restart_mse = float(np.mean(restart_result.fun**2))

        restart_improves_past_tolerance = (
            restart_mse < current_mse * (1.0 - _BASIN_REFINE_REL_TOL)
        )
        if not restart_improves_past_tolerance:
            break

        current_result = restart_result
        current_mse = restart_mse

    return current_result


def _lm_restart_from_rotated_seed(
    run_lm, current_result, rotated_velocity: ndarray, rotated_density: float
):
    restart_state = current_result.x.copy()
    restart_state[_STATE_VELOCITY_SLICE] = rotated_velocity
    if rotated_density > 0.0 and np.isfinite(rotated_density):
        restart_state[_STATE_LOG_DENSITY_IDX] = math.log(rotated_density)
    return run_lm(restart_state)
...
def _best_k_rotation_seed(
    ...
    for rotation_index in range(1, _GRID_K):
        ...
        rotated_mse = float(
            np.mean((density_scale * predicted_obs_rate - count_rate) ** 2)
        )
...
def _uncertainties_from_residual_scaled_jacobian(
    ...
    try:
        ...
        parameter_covariance = (
            residual_variance * np.linalg.pinv(lm_result.jac.T @ lm_result.jac)
        )
    except np.linalg.LinAlgError:
        return np.nan, np.nan, np.full((3, 3), np.nan)
    log_density_variance = max(
        parameter_covariance[_STATE_LOG_DENSITY_IDX, _STATE_LOG_DENSITY_IDX], 0.0
    )
    log_temperature_variance = max(
        parameter_covariance[_STATE_LOG_TEMPERATURE_IDX, _STATE_LOG_TEMPERATURE_IDX],
        0.0,
    )
    return (
        float(density * np.sqrt(log_density_variance)),
        float(temperature * np.sqrt(log_temperature_variance)),
        parameter_covariance[_STATE_VELOCITY_SLICE, _STATE_VELOCITY_SLICE],
    )
...
def _evaluate_rotated_mse(
    ...
    spin_axis = _average_spin_axis_in_rtn(rotation_matrices)
    rotated_velocity = _rotate_vector_about_axis(bulk_velocity, spin_axis, theta)
