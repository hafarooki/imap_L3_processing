...
the xarray reference script) and overlays the production kernel
`calculate_coincidence_rate` evaluated at the same parameters, with the
per-voltage relative error below.

Output: docs/swapi/figures/pui_xarray_reference.png
Usage:  uv run python docs/swapi/figure_src/plot_pui_reference_comparison.py
"""
...
from figure_utils import FIGURES_DIR, NOMINAL_EPOCH_TT2000, save_figure
...
def main():
    ...
    axes[0].set_title(
        f"PUI He coincidence rate — m/q={_HELIUM_MASS_PER_CHARGE_M_P_PER_E:.3f}\n"
        ...
        f"cooling index={SWAPI_PUI_COOLING_INDEX:.1f}, v_cutoff={_CUTOFF_SPEED_KMS:.0f} km/s, "
    ...
    axes[0].loglog(
        voltage_v, production_rate_hz, ".", color="red", label="Optimized", markersize=4
    )
    ...
    output_path = FIGURES_DIR / "pui_xarray_reference.png"
