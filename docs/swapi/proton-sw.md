...
![SWAPI proton fit on real L2 data: 5 individual sweeps and the 5-sweep average, with the production fit (all 71 science bins) and a coarse-only fit (62 bins) overlaid on the measured count rate](figures/real_data_fit.png)
...
![Monte Carlo validation: distribution of fitted parameters vs. distribution of estimated sigmas across 1000 noise realizations of one fixed solar-wind ground truth, under Poisson + log-normal + noise-floor noise](figures/uncertainty_mc.png)
