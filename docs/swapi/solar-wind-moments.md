...
- [x] Check uncertainty estimation soundness (Poisson coverage test passes at rtol=0.1 with N=500)
- [x] Demonstrate convergence of solution in variety of SW conditions (100-case synthetic benchmark, 0 bad fits; cold plasma transverse velocity recovery unit tests)
- [ ] Compare with WIND Maxwellian fits (not sure if the OMNI is using moments instead)
- [x] Try including Poisson statistics (Poisson sigma validated via coverage test at rtol=0.1, N=500)
