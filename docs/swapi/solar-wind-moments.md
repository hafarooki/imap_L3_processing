...
Integration limits are centered on $(\theta_b, \phi_b)$ and clamped per region to the active extent of that region's passband. The half-width is where the Maxwellian drops below $\varepsilon$:
$$\Delta\alpha = \arccos\!\left(\mathrm{clip}\!\left(\frac{v_\text{th}^2 \ln\varepsilon}{v_0 v_b} + 1;\; -1,\; 1\right)\right),$$
with $\varepsilon_\text{SG} = 10^{-3}$ and $\varepsilon_\text{OA} = 10^{-6}$ (the OA cutoff is two orders of magnitude tighter because OA carries the bulk of the count rate — its azimuthal transmission is $\sim 10^3 \times$ larger than SG's, so the same fractional truncation in OA leaks far more absolute counts). The integration window is $[\theta_b - \Delta\alpha,\; \theta_b + \Delta\alpha]$ in elevation and $[\phi_b - \Delta\alpha,\; \phi_b + \Delta\alpha]$ in azimuth, then clamped per region:
...
Both elevation ranges match the active extent of the per-region passband grid (the SG passband collapses above $\sim 6.5°$ and below $\sim −10.5°$, while OA reaches further). **Integration-algorithm fix:** an earlier version used a single elevation clip of $[-15°, 15°]$ for all regions and relied on `interpolate_passband` returning 0 outside the grid extent. That introduced a sharp transition at the passband edge that the trapezoid rule could not resolve — boundary samples contributed nothing while still consuming part of the trapezoid budget, and the discontinuity degraded convergence from $O(N^{-2})$ to $O(N^{-1})$. With the new bounds the integrand is smooth across the entire integration domain, and far fewer points are needed for the same accuracy (see the reduced $N$ values below).

### Speed limits

The speed integration window is computed per elevation by clamping the Maxwellian's effective support $[v_b - 5v_\text{th},\; v_b + 5v_\text{th}]$ (the integrand is suppressed by $\exp(-25/2) \approx 10^{-6}$ at the boundary) into the passband's per-elevation speed-ratio support $[r_\text{min}(\theta),\, r_\text{max}(\theta)]\,v_0$ — the blue closed curves in the passband boundary figure:
$$v_\text{min}(\theta) = \mathrm{clip}\!\left(v_b - 5v_\text{th};\; r_\text{min}(\theta)\,v_0,\; r_\text{max}(\theta)\,v_0\right), \qquad v_\text{max}(\theta) = \mathrm{clip}\!\left(v_b + 5v_\text{th};\; r_\text{min}(\theta)\,v_0,\; r_\text{max}(\theta)\,v_0\right).$$
The boundary curves $r_\text{min}(\theta)$, $r_\text{max}(\theta)$ are evaluated from the per-region `min_*_boundary` / `max_*_boundary` arrays (one grid step outside the nonzero passband region) by `_eval_boundary` in `calculate_proton_solar_wind_moments.py`. The window adapts per elevation but is independent of azimuth — the Maxwellian's effective support uses the global $v_b$ rather than the angle-dependent $v_b\cos\alpha$. When the Maxwellian's effective support falls entirely outside the passband at a given elevation, both clamps return the same passband edge: the speed grid collapses to zero width and the elevation contributes nothing. As with the angular bounds, restricting the speed domain to the smooth nonzero region (rather than integrating over a region where the integrand discontinuously drops to zero) is what gives the trapezoid rule its $O(N^{-2})$ convergence.
...
The integral is evaluated as nested elevation $\to$ azimuth $\to$ speed loops. The per-elevation passband row $v^3 P(v/v_0, \theta)$ is precomputed once and reused across all azimuths in the inner loop (the azimuth dependence enters only through $\cos\alpha$ in the Maxwellian, not through the passband). The trapezoid rule uses $N_\text{elev} = 21$, $N_\text{az,SG} = 21$, $N_\text{az,OA} = 41$, $N_\text{speed} = 11$ uniformly spaced points per dimension, with weights precomputed by `_trapz_weights` and accumulated in-place — there is no $O(N^3)$ intermediate integrand tensor. JIT-compiled with Numba. See `calculate_integral(PassbandGrid, SWParams)` in `calculate_proton_solar_wind_moments.py`.

Representative model spectra are shown below for six cases chosen to exercise the integrator's edges. The on-axis nominal (450 km/s, T = 10 eV) and on-axis hot (T = 100 eV) configurations both agree with the reference to under 2%: the Gaussian is well-resolved by the trapezoid rule, and in the hot case the Maxwellian's effective speed support exceeds the passband at every elevation so the speed window is clamped to the smooth passband edges where the trapezoid rule converges as $O(N^{-2})$. The cold case (T = 1 eV) is the worst-resolved on-axis case (3.2% error): the narrow Gaussian has the most curvature across the 11-point speed grid, so the trapezoid rule's leading error term — proportional to the integrand's second derivative — is the largest of the on-axis cases. The off-axis elevation case ($\theta_b = 9°$, past the SG passband edge at 6.5°) tests per-region elevation clamping: SG contributes negligibly while OA carries the signal, agreement under 0.5%. The off-axis azimuth case ($\phi_b = 18°$, just inside the SG boundary at 20°) is the worst case overall (18.8% error); the bulk lies right next to the SG/OA transition at $\pm 20°$, where the azimuthal transmission rises by five orders of magnitude across 10°, and the optimized integrator's 41 OA azimuth points cannot resolve that transition as densely as the reference (which uses 0.1° spacing through the transition zone). Finally, the fast solar wind case (700 km/s, T = 20 eV) exercises the passband polynomial fits at higher beam energies.
