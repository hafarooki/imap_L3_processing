...
This works for any spin axis orientation, not just radial. Evaluate the residual at $\hat{\mathbf{x}}' = (\log n, \log T, \mathbf{v}_b')$. If $\chi^2(\hat{\mathbf{x}}') < \chi^2(\hat{\mathbf{x}})$, LM was in the wrong basin — re-run LM from $\hat{\mathbf{x}}'$ to land at the correct local minimum. The cost is one extra residual evaluation in the typical case, plus one extra LM run for the $\sim 1\%$ of cases that need it. On a 1000-case synthetic benchmark covering bulk speeds 300–800 km/s and temperatures 2–50 eV, this fixes 8/8 sign-flip failures with no false positives.
