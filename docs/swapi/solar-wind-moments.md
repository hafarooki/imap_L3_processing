...
The fixed $v_T = -30$ km/s offset avoids placing the optimizer on the symmetry saddle between the two basins (see Wrong-basin detection below). Preserving $|\mathbf{v}_b^{(0)}| = v_b$ keeps the initial speed consistent with the Gaussian fit. The optimizer in Step 3 recovers the true transverse components from the small spin-phase modulation of the bulk azimuth/elevation in the instrument frame.
...
$$|\mathbf{v}| = |\mathbf{u}|, \qquad \phi_c = \arctan2(u_1,\, u_0), \qquad \phi_d = \arccos\!\left(\frac{u_2}{|\mathbf{u}|}\right).$$
