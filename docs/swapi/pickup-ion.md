...
![real example](figures/pui_flight_model_comparison.png)

*Generated by `docs/swapi/figure_src/plot_pui_flight_model_comparison.py`.*
...
> ![PUI distribution f_PUI(v') versus comoving speed for cooling indices 1.5, 2.5, 3.5 at a 450 km/s cutoff, showing the hard cutoff at v_b and the filled-shell shape set by the cooling-index/neutral-density competition.](figures/pui_distribution.png)
>
...
![Closed-form φ-inversion result compared with a numerical shell-integral reference for H(v', V) at V=5000 V, He+ (m/q=3.973), bulk=450 km/s at (az=5°, el=−10°)](figures/collapsed_response_grid.png)
...
![PUI coincidence rate at one ESA voltage versus cutoff speed $v_b$: the hard-Heaviside model is a discontinuous staircase across $v'$ grid edges, while the production trapezoidal partial cell is continuous and tracks the fine-grid reference.](figures/pui_cutoff_staircase.png)

*Generated by `docs/swapi/figure_src/plot_pui_cutoff_staircase.py`.*


The plot below shows the resulting model coincidence-rate spectrum across the coarse ESA voltage table at a representative parameter set.
The rate climbs as the passband sweeps up through the pickup ion distribution, then drops sharply once the passband moves past the cutoff speed.

![Modeled PUI He coincidence-rate spectrum across the coarse ESA voltage table at a representative parameter set, showing the rolloff above the cutoff speed.](figures/pui_coincidence_rate_spectrum.png)

*Generated by `docs/swapi/figure_src/plot_pui_coincidence_rate_spectrum.py`.*
