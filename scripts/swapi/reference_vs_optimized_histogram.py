...
def main():
    ...
    reference_bin_idx = np.digitize(references_valid, reference_edges) - 1  # 0..n_reference_bins-1
    n_reference_bins = len(reference_labels)

    # Ratio bins centered on 1.0, log-spaced symmetric around 1
    ratio_edges = np.array([
        0.0, 0.5, 0.7, 0.85, 0.92, 0.96, 0.98, 0.99, 1.0,
        1.01, 1.02, 1.04, 1.08, 1.15, 1.30, 1.5, 2.0, np.inf,
    ])
    ...
    for r, t in zip(ratios, reference_bin_idx):
        ri = np.clip(np.searchsorted(ratio_edges, r, side='right') - 1, 0, n_ratio_bins - 1)
    ...
    bar_colors = [cmap(i / max(1, n_reference_bins - 1)) for i in range(n_reference_bins)]
    ...
    for j in range(n_reference_bins):
        ax.bar(x, counts[:, j], width=bar_width, bottom=bottom,
               color=bar_colors[j], label=reference_labels[j], edgecolor='white', linewidth=0.4)
        bottom += counts[:, j]

    # Tick labels are the ratio bin edges (e.g., "0.96–0.98")
    bin_label = lambda lo, hi: (
        f"<{hi:g}" if lo == 0 else
        f">{lo:g}" if not np.isfinite(hi) else
        f"{lo:g}–{hi:g}"
    )
    ax.set_xticks(x)
    ax.set_xticklabels([bin_label(ratio_edges[i], ratio_edges[i + 1]) for i in range(n_ratio_bins)],
                       rotation=45, ha='right', fontsize=9)

    # Highlight the "ratio = 1" bin
    one_idx = np.searchsorted(ratio_edges, 1.0) - 1
    ax.axvline(one_idx + 0.5, color='red', linestyle=':', linewidth=1, alpha=0.7,
               label='ratio = 1')
    ...
    for i, total in enumerate(totals):
        if total == 0:
            continue
        ax.text(i, total + y_pad, str(int(total)),
